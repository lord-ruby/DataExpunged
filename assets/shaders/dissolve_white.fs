...
extern MY_HIGHP_OR_MEDIUMP vec2 dissolve_white;
...
vec4 dissolve_mask(vec4 tex, vec2 texture_coords, vec2 uv)
{
    ...
    return res > adjusted_dissolve
        ? vec4(shadow ? vec3(0.,0.,0.) : tex.xyz, shadow ? tex.a*0.3: tex.a)
        : (tex.a > 0.01 ? vec4(1.0, 1.0, 1.0, 1.0) : vec4(0.0, 0.0, 0.0, 0.0));
}
